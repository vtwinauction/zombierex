...
function CreatorPublicPage() {
  ...
  async function doSub(tierId: string | null) {
    ...
    setOk(null);
    try {
      await sub({ data: { creator_id: creatorId, tier_id: tierId } });
      setOk("Subscribed");
  ...
  async function doTip() {
    ...
    try {
      await tip({
        data: { creator_id: creatorId, amount_cents: tipAmt, message: tipMsg || undefined },
      });
      setOk("Tip sent");
      setTipOpen(false);
      setTipMsg("");
    ...
    try {
      await collab({
        data: {
          ...
          subject: collabSubj,
          message: collabMsg,
          budget_cents: typeof collabBudget === "number" ? collabBudget : undefined,
        },
      });
      setOk("Request sent");
      setCollabOpen(false);
  ...
  return (
    <PullToRefresh onRefresh={() => qc.invalidateQueries({ queryKey: ["creator", id] })}>
      <div className="pb-24">
        {isLoading && (
          <p className="px-4 pt-6 mono-tag" style={{ color: "var(--color-titanium)" }}>
            LOADING…
          </p>
        )}
        {!isLoading && !c && (
          <div
            className="mx-4 mt-6 border border-dashed p-6 text-center"
          ...
          >
            ...
            <Link
              to="/creators"
              className="mono-tag font-bold mt-3 inline-block"
          ...
          <>
            <div className="relative">
              <div className="h-56 w-full">
                {c.profile?.avatar_url ? (
                  <img
                    ...
                    style={{ filter: "grayscale(0.25) contrast(1.1)" }}
                  />
                ) : (
              ...
              <div className="absolute inset-x-4 bottom-3">
                ...
                <h1
                  className="serif mt-2 text-3xl italic leading-tight"
            ...
            <div
              className="grid grid-cols-3 gap-px mx-4 mt-4 border"
            ...
            <div className="px-4 pt-4 grid grid-cols-2 gap-2">
              ...
              {c.accepts_collabs && (
                <button
                  ...
                  className="tap border font-bold"
                  style={{
            ...
            {ok && (
              <p className="mx-4 mt-3 mono-tag" style={{ color: "var(--color-neon)" }}>
                {ok}
              </p>
            )}
            {err && (
              <p className="mx-4 mt-3 mono-tag" style={{ color: "#ff3d3d" }}>
                {err}
              </p>
            )}

            <div className="px-4 pt-6">
              <p className="mono-tag font-bold" style={{ color: "var(--color-titanium)" }}>
                SUBSCRIPTION TIERS
              </p>
              <div className="mt-2 space-y-2">
                {(c.tiers ?? []).length === 0 && (
                  ...
                  >
                    ...
                    <span className="mono-tag font-bold" style={{ color: "var(--color-neon)" }}>
                      {c.my_subscription ? "SUBSCRIBED" : "SUBSCRIBE ▸"}
                    </span>
                  </button>
                )}
                {(c.tiers ?? []).map((t: any) => {
                  const active =
                    c.my_subscription?.tier_id === t.id && c.my_subscription?.status === "active";
                  return (
                    <button
                      key={t.id}
                      disabled={busy === "sub"}
                    ...
                    >
                      ...
                      {t.description && (
                        <p className="mt-1 text-xs" style={{ color: "var(--color-silver)" }}>
                          {t.description}
                        </p>
                      )}
                      <p
                        className="mt-2 mono-tag font-bold"
            ...
            <div className="grid grid-cols-4 gap-2">
              {[100, 500, 1000, 2500].map((n) => (
                <button
                  ...
                  style={{
                    borderColor: tipAmt === n ? "var(--color-neon)" : "var(--color-hair-strong)",
                    color: "var(--color-ink)",
                  }}
                >
                  {formatMoney(n)}
        ...
        {collabOpen && (
          <Modal onClose={() => setCollabOpen(false)} title="Request collab">
            <input
              placeholder="Subject"
              value={collabSubj}
              onChange={(e) => setCollabSubj(e.target.value)}
            ...
            <input
              type="number"
              placeholder="Budget cents (optional)"
              value={collabBudget}
              onChange={(e) => setCollabBudget(e.target.value ? Number(e.target.value) : "")}
              className="mt-3 w-full border px-3 py-2 text-sm"
              style={{
                background: "rgba(255,255,255,0.03)",
                borderColor: "var(--color-hair-strong)",
            ...
            />
            <button
              disabled={busy === "collab" || !collabSubj.trim() || !collabMsg.trim()}
              onClick={doCollab}
              className="btn-neon mt-4 w-full"
              style={{ padding: "12px", fontSize: 11 }}
            >
...
function Modal({
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-md p-4 border-t"
        style={{ background: "var(--color-obsidian, #0a0a0a)", borderColor: "var(--color-neon)" }}
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between">
          <h2 className="serif text-xl italic" style={{ color: "var(--color-ink)" }}>
            {title}
          </h2>
          <button
            onClick={onClose}
