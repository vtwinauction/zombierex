  ...
  head: () => ({
    meta: [
      { title: "ZOMBIEREX — The World's Automotive Social Network" },
      {
        name: "description",
        content:
          "Cars, motorcycles, drag racing, drifting and car shows in one app. Share builds, verify times, plan routes, buy and sell parts, and find your crew.",
      },
      { property: "og:title", content: "ZOMBIEREX — The World's Automotive Social Network" },
      {
        property: "og:description",
        content:
          "Cars, motorcycles, drag racing, drifting and car shows in one app. Join 128,000+ riders and drivers worldwide.",
      },
      ...
      <div className="pb-24">
          ...
          <div className="mt-3 grid grid-cols-5 gap-2">
            {QUICK_ACTIONS.map((a) => (
              ...
              >
                ...
                <span className="text-[11px] font-semibold" style={{ color: "var(--color-ink-1)" }}>
                  {a.label}
                </span>
              </Link>
            ))}
        ...
        <section className="mt-8">
          <div className="mb-3 flex items-end justify-between px-4">
            <div>
              ...
              <h2
                className="serif text-[20px] italic leading-none"
                style={{ color: "var(--color-ink)" }}
              >
                Recent chats
              </h2>
            </div>
            <Link to="/messages" className="mono-tag" style={{ color: "var(--color-neon)" }}>
              Open →
            </Link>
          </div>
        ...
        <section className="mt-8">
          <div className="mb-3 flex items-end justify-between px-4">
            ...
            <Link to="/search" className="mono-tag" style={{ color: "var(--color-neon)" }}>
              Discover →
            </Link>
          </div>
        ...
        <section className="mt-8 space-y-10">
          {feedPosts.map((p: any, idx: number) => (
            <div key={p.id}>
              <article className="rise" style={{ animationDelay: `${idx * 40}ms` }}>
                ...
                <div className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3 px-4 pb-3">
                  <div className="story-ring shrink-0">
                    <div
                      style={{ background: "var(--color-paper-0)", padding: 2, borderRadius: 999 }}
                    >
                      <img
                        src={p.user.avatar}
                        alt=""
                ...
                <FeedMedia
                  ...
                  vehicle={p.vehicle}
                  onDoubleTap={() => {
                    /* optimistic like handled by InteractionBar tap; heart burst rendered inside */
                  }}
                />
                ...
                <div className="mt-4 px-4">
                  <p
                    ...
                    style={{ color: "var(--color-ink-0)" }}
                  >
                    <span className="font-semibold">{p.user.handle}</span> {p.caption}
                  </p>
              ...
              {idx > 0 &&
                idx % 3 === 0 &&
                sponsored.data?.[Math.floor(idx / 3) % (sponsored.data?.length || 1)] && (
...
function RailBtn({
  ...
  return (
    <button className="tap flex flex-col items-center gap-1">
      ...
      <span className="mono-num text-[10.5px] font-semibold" style={{ color: "white" }}>
        {count}
      </span>
    </button>
  );
...
function IconChip({ children }: { children: React.ReactNode; label?: string }) {
  return (
    <button
      ...
      style={{
        background:
          "linear-gradient(140deg, var(--color-neon) 0%, var(--color-neon-deep) 55%, var(--color-ember) 130%)",
        color: "var(--color-obsidian)",
...
function BluetoothPill() {
  ...
  async function onPair() {
    ...
    try {
      ...
      setState("linked");
      try {
        localStorage.setItem("zrex:bt", JSON.stringify({ name: dn, at: Date.now() }));
      } catch {
        /* noop */
      }
    } catch {
      ...
      <Bluetooth
        ...
        style={
          scanning
            ...
            : linked
              ? { filter: "drop-shadow(0 0 5px rgba(124,255,63,0.6))" }
              : undefined
        }
      />
    ...
    <div className="relative select-none" onClick={onClick}>
      ...
      {vehicle && (
        ...
        >
          ...
          <span className="mono-num shrink-0 text-[10px]" style={{ color: "var(--color-neon)" }}>
            {vehicle.hp}hp
          </span>
        </div>
      )}
