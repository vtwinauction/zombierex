...
export const Route = createFileRoute("/_authenticated/owner/finance/audit")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Immutable audit trail of financial rule changes and admin actions.",
      },
...
function AuditPage() {
  ...
  return (
    <div className="space-y-3 p-5">
      ...
      {q.data?.length === 0 && (
        <p className="text-xs opacity-50">No financial changes recorded yet.</p>
      )}
      {((q.data ?? []) as any[]).map((row) => (
        <div key={row.id} className="card-surface p-3">
          ...
          {row.meta && Object.keys(row.meta).length > 0 && (
            <pre className="mt-2 overflow-x-auto text-[10px] opacity-50">
