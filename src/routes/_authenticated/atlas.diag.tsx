...
export const Route = createFileRoute("/_authenticated/atlas/diag")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Live OBD-II telemetry, trouble codes, and battery health over Web Bluetooth.",
      },
      { property: "og:title", content: "Bike Diagnostics — Zombierex" },
      {
        property: "og:description",
        content: "Pair a BLE OBD-II dongle to stream live engine data and read fault codes.",
      },
    ],
  }),
  component: DiagPage,
    ...
    <div className="rounded-2xl border border-border bg-card p-4">
      ...
      <div className="flex items-baseline gap-1">
        <div
          ...
          style={{ color: warn ? "#dc2626" : "var(--color-ink-0)" }}
        >
          {r ? r.value : "—"}
        </div>
...
function DiagPage() {
  const [state, setState] = useState<
    "idle" | "connecting" | "ready" | "demo" | "error" | "disconnected"
  >("idle");
  ...
  const appendLog = (l: string) =>
    setLog((prev) => [`${new Date().toLocaleTimeString()}  ${l}`, ...prev].slice(0, 40));
  ...
  const connect = async () => {
    ...
    setReadings({});
    if (!supported) {
      toast.error("Web Bluetooth is not available on this browser");
      startDemo();
      return;
    }
  ...
  const scanDtcs = async () => {
    if (!clientRef.current) {
      toast.error("Connect a dongle first");
      return;
    }
    appendLog("Reading DTCs…");
    try {
      const codes = await clientRef.current.readDtcs();
      if (!codes.length) appendLog("No codes stored ✓");
    } catch {
      toast.error("DTC read failed");
    }
  };
  ...
  const clearDtcs = async () => {
    ...
    if (
      !(await confirmDialog({
        ...
        description: "Resets the check-engine light and freeze-frame data.",
        destructive: true,
    ...
    <div className="min-h-svh bg-background text-foreground pb-24">
      <header className="sticky top-0 z-20 bg-background/95 backdrop-blur border-b border-border">
        <div className="flex items-center gap-3 px-4 py-3">
          <Link
            to="/atlas"
            className="tap grid h-10 w-10 place-items-center rounded-full border border-border"
          >
            <ArrowLeft size={18} />
          </Link>
      ...
      <div className="px-4 pt-4 space-y-4">
        <div className="rounded-2xl border border-border bg-card p-4">
          ...
          <p className="text-xs opacity-70 mb-3">
            Compatible with Vgate iCar Pro BLE, VEEPEAK BLE+, LELink and other ELM327 BLE adapters.
            Classic-Bluetooth SPP dongles are not supported by the browser.
          </p>
          <div className="flex gap-2">
            {!connected ? (
              <button
                onClick={connect}
                className="tap flex-1 grid place-items-center rounded-xl py-3 font-semibold"
                style={{
                  background: "var(--color-neon, #00c853)",
                  color: "var(--color-obsidian, #0f1115)",
                }}
              >
                <div className="flex items-center gap-2">
                  <Play size={16} /> Pair &amp; Connect
                </div>
              </button>
            ) : (
              <button
            ...
            <button
              onClick={startDemo}
              className="tap grid place-items-center rounded-xl px-4 py-3 font-semibold border border-border"
            >
              <div className="flex items-center gap-2">
                <Zap size={16} /> Demo
              </div>
          ...
          {!supported && (
            <div className="mt-3 text-[11px] opacity-70">
              Web Bluetooth isn't available here (iOS Safari, some in-app browsers). Use Chrome,
              Edge, or Android WebView — or run Demo mode.
            </div>
          )}
        </div>
        ...
        <div className="grid grid-cols-2 gap-2">
          {PIDS.map((p) => (
            <Tile key={p} pid={p} r={readings[p] ?? null} />
          ))}
        </div>
        ...
        <div className="rounded-2xl border border-border bg-card p-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex gap-2">
              <button
                onClick={scanDtcs}
                disabled={!connected || state === "demo"}
                className="tap grid place-items-center rounded-lg px-3 py-1.5 text-xs font-semibold border border-border disabled:opacity-40"
              >
              ...
              <button
                onClick={clearDtcs}
                disabled={!dtcs.length || state === "demo"}
                className="tap grid place-items-center rounded-lg px-3 py-1.5 text-xs font-semibold border border-border disabled:opacity-40"
              >
                <div className="flex items-center gap-1.5">
          ...
          {dtcs.length === 0 ? (
            <div className="text-xs font-mono opacity-60">
              No codes read. Pair a dongle and tap Scan.
            </div>
          ) : (
            <ul className="space-y-1.5">
              {dtcs.map((c) => (
                ...
                >
                  ...
                  <a
                    href={`https://www.google.com/search?q=OBD+${c}`}
                    target="_blank"
                    rel="noreferrer"
                    className="text-[11px] opacity-70 underline"
                  >
                    Look up
                  </a>
                </li>
        ...
        <div className="rounded-2xl border border-border bg-card p-4">
          ...
          <div className="max-h-48 overflow-auto font-mono text-[11px] leading-relaxed opacity-80">
            {log.length === 0 ? (
              <div className="opacity-50">— idle —</div>
            ) : (
