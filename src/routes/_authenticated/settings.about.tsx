...
export const Route = createFileRoute("/_authenticated/settings/about")({
  head: () => ({
    meta: [
      { title: "About · ZOMBIEREX" },
      { name: "description", content: "App version, build and credits for ZOMBIEREX." },
    ],
  }),
  component: AboutPage,
});
...
function AboutPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (a) {
          info.appVersion = a.version;
          info.appBuild = a.build;
        }
      } catch {
        /* ignore */
      }
      const devMod = await loadPlugin<{
        Device: {
          getInfo: () => Promise<{ osVersion: string; model: string; manufacturer: string }>;
        };
      }>("@capacitor/device");
      try {
        const d = await devMod?.Device.getInfo();
        if (d) {
          info.osVersion = d.osVersion;
          info.model = d.model;
  ...
  return (
    <SettingsScreen
      ...
      section="ABOUT"
      title="About ZOMBIEREX"
      subtitle="The world's premium platform for motorcycle and automotive culture."
    >
      <div className="space-y-3">
        <Card>
          <div className="grid grid-cols-2 gap-y-2 text-[13px]">
            ...
            <span style={{ color: "var(--color-ink)" }}>
              {native?.appBuild ? `${BUILD_NAME} (${native.appBuild})` : `“${BUILD_NAME}”`}
            </span>
            ...
            <span className="uppercase tracking-wider" style={{ color: "var(--color-neon)" }}>
              {runtime}
            </span>
          </div>
        </Card>
        ...
        {native && (native.model || native.osVersion) && (
          <Card>
            ...
            <div className="mt-2 grid grid-cols-2 gap-y-2 text-[13px]">
              {native.manufacturer && (
                <>
                  <span style={{ color: "var(--color-silver)" }}>Manufacturer</span>
                  <span style={{ color: "var(--color-ink)" }}>{native.manufacturer}</span>
                </>
              )}
              {native.model && (
                <>
                  <span style={{ color: "var(--color-silver)" }}>Model</span>
                  <span style={{ color: "var(--color-ink)" }}>{native.model}</span>
                </>
              )}
              {native.osVersion && (
                <>
                  <span style={{ color: "var(--color-silver)" }}>OS</span>
                  <span style={{ color: "var(--color-ink)" }}>
                    {platform()} {native.osVersion}
                  </span>
                </>
              )}
            </div>
        ...
        <Card>
          <p className="serif text-[16px] italic" style={{ color: "var(--color-ink)" }}>
            Credits
          </p>
        ...
        <Card>
          <div className="flex flex-wrap gap-3 text-[13px]">
            <Link to="/settings/terms" style={{ color: "var(--color-neon)" }}>
              Terms of service
            </Link>
            <Link to="/settings/privacy" style={{ color: "var(--color-neon)" }}>
              Privacy policy
            </Link>
            <Link to="/settings/help" style={{ color: "var(--color-neon)" }}>
              Help centre
            </Link>
          </div>
        </Card>
