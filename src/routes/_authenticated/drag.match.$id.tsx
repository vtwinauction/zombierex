...
import {
  getMatch, markMatchReady, pushMatchTelemetry, finalizeMatch,
...
export const Route = createFileRoute("/_authenticated/drag/match/$id")({
  head: ({ params }) => ({
    meta: [
      ...
      { name: "description", content: `Live head-to-head drag race on ZOMBIEREX — match ${params.id}` },
    ],
  }),
  component: MatchPage,
});
...
type Match = {
  id: string; rider_a: string; rider_b: string;
  status: "lobby" | "armed" | "countdown" | "live" | "finished" | "void";
  ready_a: boolean; ready_b: boolean;
  green_at: string | null;
  strip_mode: "eighth" | "quarter"; tree_mode: "pro" | "sportsman";
  stake_xp: number;
  winner_id: string | null; margin_s: number | null;
  rider_a_profile: any; rider_b_profile: any;
  result_a: any; result_b: any;
};

type TelRow = { rider_id: string; t_ms: number; distance_m: number; speed_kmh: number; accuracy_m: number | null };
...
function MatchPage() {
  ...
  useEffect(() => {
    const ch = supabase.channel(`match:${id}`)
      .on("postgres_changes", { event: "UPDATE", schema: "public", table: "drag_matches", filter: `id=eq.${id}` },
  ...
  useEffect(() => {
    ...
    supabase.from("drag_match_telemetry")
      .select("rider_id, t_ms, distance_m, speed_kmh, accuracy_m")
      .eq("match_id", id).order("t_ms", { ascending: true }).limit(4000)
      .then(({ data }) => { if (data) setTel(data as any); });
    const ch = supabase.channel(`match-tel:${id}`)
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "drag_match_telemetry", filter: `match_id=eq.${id}` },
        (payload) => { setTel((prev) => [...prev, payload.new as TelRow]); })
      .subscribe();
    return () => { supabase.removeChannel(ch); };
  }, [id, match?.id]);
  ...
  useEffect(() => { supabase.auth.getUser().then(({ data }) => setMeUid(data.user?.id ?? null)); void meId; }, []);
  ...
  useEffect(() => {
    ...
    watchRef.current = navigator.geolocation.watchPosition((pos) => {
      ...
      bufferRef.current.push({
        rider_id: meUid!, t_ms, distance_m: distRef.current, speed_kmh: spd, accuracy_m: pos.coords.accuracy ?? null,
      });
      ...
      if (now - lastPushRef.current > 250 && bufferRef.current.length) {
        ...
        pushFn({ data: { match_id: id, samples: samples.map((s) => ({
          t_ms: s.t_ms, distance_m: Number(s.distance_m.toFixed(2)),
          speed_kmh: Number(s.speed_kmh.toFixed(2)), accuracy_m: s.accuracy_m ?? null,
        })) } }).catch(() => null);
      }

      if (distRef.current >= target) {
        finalFn({ data: { id } }).then(() => refetch()).catch(() => null);
      }
    }, () => null, { enableHighAccuracy: true, maximumAge: 0, timeout: 10_000 });
    ...
    return () => {
      ...
      bufferRef.current = []; lastPushRef.current = 0;
      distRef.current = 0; lastPtRef.current = null;
    };
  }, [greenMs, iAmA, iAmB, meUid, id, match?.strip_mode, match?.status, pushFn, finalFn, refetch]);

  // Build lane telemetry from streams
  const laneA = useMemo(() => buildLane(match?.rider_a_profile, LANE_A, tel.filter((t) => t.rider_id === match?.rider_a)), [match?.rider_a, match?.rider_a_profile, tel]);
  const laneB = useMemo(() => buildLane(match?.rider_b_profile, LANE_B, tel.filter((t) => t.rider_id === match?.rider_b)), [match?.rider_b, match?.rider_b_profile, tel]);

  // Sync countdown display
  const [countdown, setCountdown] = useState<number | null>(null);
  useEffect(() => {
    if (!greenMs || match?.status === "finished") { setCountdown(null); return; }
    const t = setInterval(() => {
  ...
  if (!match) {
    return <div className="min-h-svh p-6 text-sm" style={{ color: "var(--color-ink-3)" }}>Loading match…</div>;
  }
  ...
  return (
    <div className="min-h-svh pb-24" style={{ background: "#050505" }}>
      <div className="px-4 pt-3 flex items-center justify-between">
        <Link to="/drag/challenge" className="tap mono-caps text-[10px]" style={{ color: "var(--color-silver)" }}>← CHALLENGES</Link>
        <span className="mono-tag" style={{ color: "var(--color-silver)", fontSize: 9 }}>
          {match.strip_mode === "quarter" ? "1/4 MI" : "1/8 MI"} · {match.tree_mode.toUpperCase()} · STAKE {match.stake_xp}
        </span>
      </div>
      ...
      {!finished && !green && (
        <div className="mx-4 mt-4 rounded-2xl border p-4 text-center"
          ...
          {countdown != null && countdown > 0 ? (
            <>
              <p className="mono-caps text-[10px] font-black" style={{ color: "#f6d84f", letterSpacing: "0.24em" }}>◆ GREEN IN</p>
              <p className="mono-num text-6xl font-black tabular-nums" style={{ color: "#00c853" }}>
                {(countdown / 1000).toFixed(1)}
              </p>
              <p className="mt-1 text-xs" style={{ color: "var(--color-ink-3)" }}>Both riders staged. Hold the line.</p>
            </>
          ) : (
            <>
              <p className="mono-caps text-[10px] font-black" style={{ color: "var(--color-silver)", letterSpacing: "0.24em" }}>◆ LOBBY</p>
              <p className="mt-1 text-sm" style={{ color: "var(--color-ink)" }}>
                {match.ready_a && match.ready_b ? "Locking in the green light…" : "Waiting for both riders to be READY."}
              </p>
              {(iAmA || iAmB) && !spectator && (
                <button
                  ...
                  style={{ padding: "12px 20px", background: meReady ? "#111" : "var(--color-neon)", color: meReady ? "#f5f5f5" : "var(--color-obsidian)", border: meReady ? "1px solid #333" : "none" }}>
                  {meReady ? "◇ UN-READY" : "◆ I'M READY"}
                </button>
              )}
              {spectator && <p className="mt-2 text-xs" style={{ color: "var(--color-ink-3)" }}>Spectating this race.</p>}
            </>
          )}
        </div>
      )}
      ...
      {(green || finished) && (
        <div className="mx-4 mt-4">
          <RaceHUD player={iAmB ? laneB : laneA} ghost={iAmB ? laneA : laneB}
            elapsedMs={greenMs ? Math.max(0, Date.now() - greenMs) : 0} finished={finished} />
        </div>
      )}
      ...
      {finished && (
        <div className="mx-4 mt-4 rounded-2xl border p-4 text-center"
          style={{ borderColor: "rgba(0,200,83,0.4)", background: "linear-gradient(120deg,#050505,#0f2015)" }}>
          <p className="mono-caps text-[10px] font-black" style={{ color: "#00c853", letterSpacing: "0.24em" }}>◆ RACE COMPLETE</p>
          {match.winner_id ? (
            <>
              <h2 className="serif mt-1 text-2xl" style={{ color: "#f5f5f5" }}>
                {match.winner_id === match.rider_a ? (match.rider_a_profile?.display_name || "Rider A") : (match.rider_b_profile?.display_name || "Rider B")} wins
              </h2>
              {match.margin_s != null && (
              ...
              {match.winner_id === meUid && (
                <p className="mt-2 text-sm" style={{ color: "#00c853" }}>+{match.stake_xp} XP</p>
              )}
            </>
          ) : (
            <p className="mt-1 text-sm" style={{ color: "var(--color-ink-3)" }}>No result — try again.</p>
          )}
          <button onClick={() => nav({ to: "/drag/challenge" })}
            className="tap mono-caps mt-4 text-[11px] font-black"
            style={{ padding: "10px 16px", background: "var(--color-neon)", color: "var(--color-obsidian)" }}>
            NEW CHALLENGE
          </button>
        </div>
      )}
      ...
      {(iAmA || iAmB) && green && !finished && (
        <div className="mx-4 mt-3 rounded-lg border p-2 text-center"
          style={{ borderColor: "var(--color-hair)", background: "#0a0a0a" }}>
          <span className="mono-tag" style={{ color: "var(--color-silver)", fontSize: 9 }}>
            YOUR GPS · {liveKmh.toFixed(0)} km/h · {gpsAcc != null ? `±${gpsAcc.toFixed(0)}m` : "acquiring"}
          </span>
        </div>
      )}
    </div>
...
function RiderPill({ p, color, ready, you }: { p: any; color: string; ready: boolean; you: boolean }) {
  return (
    <div className="rounded-xl border p-2 flex items-center gap-2"
      style={{ borderColor: ready ? color : "var(--color-hair-strong)", background: "linear-gradient(180deg,#080808,#141414)", boxShadow: ready ? `0 0 18px ${color}44` : "none" }}>
      ...
      <div className="min-w-0 flex-1">
        <p className="text-sm font-bold truncate" style={{ color: "#f5f5f5" }}>
          {p?.display_name || p?.handle || "Rider"}{you && " · YOU"}
        </p>
