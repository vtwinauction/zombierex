...
import { listPayouts, runPayoutsNow, markPayoutPaid, checkFinanceAccess } from "@/lib/finance.functions";
...
function PayoutsPage() {
  ...
  const payouts = useQuery({ queryKey: ["finance", "payouts"], queryFn: () => load({ data: undefined as any }) });
    ...
    <div className="space-y-4 p-5">
      <div className="card-surface p-4">
        ...
        <p className="mt-1 text-[12px]" style={{ color: "var(--color-silver)" }}>
          A batch sweeps every seller whose payable balance clears their withdrawal minimum. Runs automatically on the
          configured schedule; you can also trigger one now.
        </p>
        {canWrite && (
      ...
      {note && <div className="rounded px-3 py-2 text-[12px]" style={{ background: "rgba(0,200,83,0.12)" }}>{note}</div>}
      ...
      {((payouts.data ?? []) as any[]).map((p) => (
        <div key={p.id} className="card-surface p-3">
          <div className="flex items-baseline justify-between">
            ...
            <span className="mono-tag text-[10px]" style={{ color: p.status === "paid" ? "#00c853" : "var(--color-silver)" }}>
              {p.status.toUpperCase()}
            </span>
          </div>
          <p className="mono-tag mt-1 text-[10px] opacity-50">
            {p.scheduled_for ? new Date(p.scheduled_for).toLocaleDateString() : "unscheduled"} · {p.method ?? "no method"}
          </p>
          {canWrite && p.status !== "paid" && (
            <div className="mt-2 flex gap-2">
              <button className="btn-ghost text-[10px]" onClick={() => setStatus(p.id, "paid")}>Mark paid</button>
              <button className="btn-ghost text-[10px]" onClick={() => setStatus(p.id, "failed")}>Mark failed</button>
              <button className="btn-ghost text-[10px]" onClick={() => setStatus(p.id, "cancelled")}>Cancel</button>
            </div>
          )}
        </div>
      ))}
