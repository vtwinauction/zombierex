...
function BuyersPage() {
  ...
  const buyers = useQuery({ queryKey: ["finance", "buyers", q], queryFn: () => load({ data: { q: q || undefined } }) });

  return (
    <div className="space-y-4 p-5">
      <input className="zx-input" placeholder="Search buyers…" value={q} onChange={(e) => setQ(e.target.value)} />
      ...
      {(buyers.data ?? []).map((b: any) => (
        <div key={b.buyer_id} className="card-surface p-4">
          <div className="flex items-baseline justify-between">
            ...
            <span className="mono-tag text-[10px] opacity-50">{b.handle ? `@${b.handle}` : ""}</span>
          </div>
          ...
          <p className="mt-0.5 text-[10px] opacity-40">Last activity {new Date(b.last).toLocaleString()}</p>
