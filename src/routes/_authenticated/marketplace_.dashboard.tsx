...
function Dashboard() {
  ...
  async function markSold(id: string) {
    await updateFn({ data: { id, patch: {}, status: "sold" } });
    refetch();
  }
  async function archive(id: string) {
    await updateFn({ data: { id, patch: {}, status: "archived" } });
    refetch();
  }
  async function activate(id: string) {
    await updateFn({ data: { id, patch: {}, status: "active" } });
    refetch();
  }
  async function remove(id: string) {
    if (
      await confirmDialog({ title: "Delete listing?", destructive: true, confirmLabel: "Delete" })
    ) {
      await delFn({ data: { id } });
      refetch();
    }
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="mt-1 flex flex-wrap gap-1">
                    {l.status === "active" && (
                      <MiniBtn onClick={() => markSold(l.id)}>SOLD</MiniBtn>
                    )}
                    {l.status === "active" && (
                      <MiniBtn onClick={() => archive(l.id)}>ARCHIVE</MiniBtn>
                    )}
                    {l.status !== "active" && (
                      <MiniBtn onClick={() => activate(l.id)}>RE-LIST</MiniBtn>
                    )}
                    <MiniBtn onClick={() => remove(l.id)} danger>
                      DELETE
                    </MiniBtn>
                  </div>
                </div>
...
function MiniBtn({
  ...
  return (
    <button
      onClick={onClick}
      className="tap border px-2 py-1 mono-tag font-bold"
      style={{
        borderColor: danger ? "#ff3d3d" : "var(--color-hair-strong)",
        color: danger ? "#ff3d3d" : "var(--color-ink)",
      }}
    >
      {children}
    </button>
