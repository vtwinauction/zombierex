...
export const Route = createFileRoute("/_authenticated/referrals")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Share your invite code and earn XP for every rider who joins ZOMBIEREX.",
      },
      { property: "og:title", content: "Invite Riders · ZOMBIEREX" },
      {
        property: "og:description",
        content: "Every rider you bring in earns you XP toward the next tier.",
      },
    ],
  }),
...
function ReferralsPage() {
  ...
  async function share() {
    ...
    if (navigator.share) {
      try {
        await navigator.share({ title: "ZOMBIEREX", text, url: shareUrl });
        return;
      } catch {
        /* user cancelled */
      }
    ...
    >
      <div className="pb-24">
        <div className="flex items-end justify-between px-4 pt-6">
          ...
          <Link to="/rewards" className="mono-tag" style={{ color: "var(--color-titanium)" }}>
            REWARDS ▸
          </Link>
        </div>
        ...
        >
          ...
          {q.isLoading ? (
            <p className="mono-tag mt-4" style={{ color: "var(--color-titanium)" }}>
              LOADING…
            </p>
          ) : code ? (
            <>
              <p
                className="serif mt-2 text-4xl italic tracking-wider"
                style={{ color: "var(--color-ink)" }}
              >
                {code}
              </p>
              <p className="mt-3 text-[12px] break-all" style={{ color: "var(--color-silver)" }}>
                {shareUrl}
              </p>
        ...
        >
          <p className="mono-tag mb-3" style={{ color: "var(--color-titanium)" }}>
            HOW IT WORKS
          </p>
          <ol className="space-y-2 text-[13px]" style={{ color: "var(--color-ink)" }}>
            <li>
              <span style={{ color: "var(--color-neon)" }}>1.</span> Share your code with a rider
              you trust.
            </li>
            <li>
              <span style={{ color: "var(--color-neon)" }}>2.</span> They sign up and enter your
              code during onboarding.
            </li>
            <li>
              <span style={{ color: "var(--color-neon)" }}>3.</span> You earn{" "}
              <strong>+150 XP</strong>, they get a welcome bonus.
            </li>
          </ol>
        </section>
...
function Stat({ label, value }: { label: string; value: number | string }) {
  return (
    <div
      ...
      style={{ borderColor: "var(--color-hair-strong)", background: "var(--color-graphite)" }}
    >
      <p className="serif text-2xl italic" style={{ color: "var(--color-ink)" }}>
        {value}
      </p>
