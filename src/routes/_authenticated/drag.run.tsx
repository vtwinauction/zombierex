  ...
  head: () => ({
    meta: [
      { title: "New GPS Drag Run · ZOMBIEREX" },
      {
        name: "description",
        content:
          "Record a GPS-verified drag run with vehicle setup, live timing, anti-cheat checks and a permanent performance record.",
      },
      { property: "og:title", content: "New GPS Drag Run · ZOMBIEREX" },
      {
        property: "og:description",
        content: "Record a GPS-verified drag run with live timing and verification.",
      },
...
function NewRun() {
  ...
  const mut = useMutation({
    ...
    onSuccess: async (r: any) => {
      ...
      try {
        setCoaching(await coach({ data: { id: r.id } }));
      } catch (e) {
        console.warn("Coach failed", e);
      } finally {
      ...
      <div className="px-4 pt-4">
            ...
            <div className="mt-4 space-y-3">
              <div className="grid grid-cols-2 gap-2">
                {(["motorcycle", "car"] as const).map((k) => (
                  <button
                    key={k}
                    onClick={() => setVehicle((v) => ({ ...v, vehicle_kind: k }))}
                    className="tap rounded-lg border p-3 text-sm font-bold"
                    style={{
                      borderColor:
                        vehicle.vehicle_kind === k ? "var(--color-neon)" : "var(--color-hair)",
                      background:
                        vehicle.vehicle_kind === k
              ...
              <Field
                ...
                onChange={(v) => setVehicle((s) => ({ ...s, vehicle_name: v }))}
                placeholder="e.g. Yamaha R1 2024"
              />
              <Field
                label="Engine (cc)"
                value={vehicle.engine_cc}
                onChange={(v) => setVehicle((s) => ({ ...s, engine_cc: v.replace(/[^0-9]/g, "") }))}
                placeholder="998"
                inputMode="numeric"
              />
              <Select
                ...
                onChange={(v) => setVehicle((s) => ({ ...s, aspiration: v as any }))}
                options={[
              ...
              <Select
                ...
                onChange={(v) => setVehicle((s) => ({ ...s, fuel_type: v as any }))}
                options={[
                  ["gasoline", "Gasoline"],
                  ["diesel", "Diesel"],
                  ["e85", "E85"],
              ...
              <div className="grid grid-cols-2 gap-2">
                <Field
                  ...
                  onChange={(v) => setVehicle((s) => ({ ...s, weight_class: v }))}
                  placeholder="Supersport"
                />
                <Field
                  label="Weight (kg)"
                  value={vehicle.weight_kg}
                  onChange={(v) =>
                    setVehicle((s) => ({ ...s, weight_kg: v.replace(/[^0-9]/g, "") }))
                  }
                  placeholder="200"
                  inputMode="numeric"
                />
              ...
              <Field
                ...
                onChange={(v) => setVehicle((s) => ({ ...s, modifications: v }))}
                placeholder="Full exhaust, ECU flash…"
              />
            </div>
            <Link
              to="/drag/run"
        ...
        {step === "record" && (
          <>
            ...
            <p className="mt-1 text-sm" style={{ color: "var(--color-ink-3)" }}>
              Arm the timer, then launch. Run auto-detects on 8 km/h and auto-stops when you coast
              down.
            </p>
            <LiveGPSHUD
              kmh={rec.liveKmh}
            ...
            />
            {rec.error && (
              <div className="mt-3 border border-red-500/40 bg-red-500/10 p-2 text-xs text-red-200">
                {rec.error}
              </div>
            )}
            <div className="mt-4 flex gap-2">
              {!rec.armed && (
                <button
                  onClick={rec.arm}
                  className="tap flex-1 rounded-lg py-3 mono-caps text-sm font-black"
                  style={{ background: "var(--color-neon)", color: "var(--color-obsidian)" }}
                >
                  ARM TIMER
                </button>
            ...
            <button
              onClick={rec.reset}
              className="tap mt-3 w-full text-xs"
        ...
        {step === "submitting" && (
          <div className="py-16 text-center">
            <p className="mono-caps text-sm font-black" style={{ color: "var(--color-neon)" }}>
              VERIFYING…
            </p>
            <p className="mt-2 text-sm" style={{ color: "var(--color-ink-3)" }}>
              Recomputing splits from GPS trace and running anti-cheat checks.
            </p>
          </div>
        )}
          ...
          <div className="py-8">
            ...
            >
              <div className="flex items-center justify-between">
                ...
                {coaching?.grade && (
                  <span
                    className="mono-num text-xl font-black"
                    style={{ color: "var(--color-neon)" }}
                  >
                    {coaching.grade}
                  </span>
                ...
                <div className="mt-2 space-y-2 text-[13px]" style={{ color: "var(--color-ink)" }}>
                  ...
                  {Array.isArray(coaching.tips) && coaching.tips.length > 0 && (
                    <ul className="mt-2 list-disc pl-5" style={{ color: "var(--color-ink-2)" }}>
                      {coaching.tips.map((t: string, i: number) => (
                        <li key={i}>{t}</li>
                      ))}
                    </ul>
                  )}
            ...
            <button
              onClick={() => nav({ to: "/drag/$id", params: { id: result.id } })}
...
function LiveGPSHUD({
  ...
  const stateColor = recording
    ? "var(--color-heat, #ff3b30)"
    : armed
      ? "var(--color-neon)"
      : "var(--color-silver)";
  return (
    ...
    >
      <div className="flex items-center justify-between">
        ...
        <span className="mono-tag" style={{ color: "var(--color-silver)", fontSize: 10 }}>
          {points} pts
        </span>
      </div>
      <div className="mt-2 flex items-end justify-center gap-2">
        <span
          className="mono-num font-black tabular-nums"
          style={{
            fontSize: 96,
            lineHeight: 1,
            color: "#fff",
            textShadow: "0 0 24px rgba(0,200,83,0.35)",
          }}
        >
          {kmh.toFixed(0)}
        </span>
        <span
          className="mono-caps pb-3"
          style={{ color: "var(--color-silver)", fontSize: 12, letterSpacing: "0.3em" }}
        >
          km/h
        </span>
      </div>
      ...
      >
        <div
          style={{
            width: `${Math.min(100, (kmh / 300) * 100)}%`,
            height: "100%",
