...
export const Route = createFileRoute("/_authenticated/rewards")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Your XP, streaks, achievements, challenges, and leaderboard standing.",
      },
      { property: "og:title", content: "Rewards · ZOMBIEREX" },
      {
        property: "og:description",
        content: "Track your XP, streaks, achievements, and leaderboard rank.",
      },
  ...
  const checkin = useMutation({
    ...
    onSuccess: (r: any) => {
      if (r.alreadyCheckedIn)
        toast("Already checked in today", { description: `Streak: ${r.streak} days` });
      ...
      <div className="min-h-screen bg-background text-foreground pb-24">
        ...
        <div className="mx-4 mt-3 grid grid-cols-3 gap-3">
          ...
          <StatTile
            ...
            label="BADGES"
            value={`${s?.achievements_unlocked ?? 0}`}
          />
        ...
        >
          ...
          <span className="text-[11px] tracking-[0.2em] text-[hsl(var(--rex-signal,140_90%_50%))]">
            CLAIM →
          </span>
        </button>
          ...
          {challenges.isLoading ? (
            ...
            <div className="space-y-2">
              {(challenges.data ?? []).map((c: any) => {
                const pct = Math.min(
                  100,
                  Math.round(((c.progress ?? 0) / Math.max(1, c.goal_count)) * 100),
                );
                  ...
                  <div key={c.id} className="rounded-xl border border-border bg-card p-3">
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <div className="text-right text-[11px] shrink-0">
                        <div className="tracking-[0.2em] text-[hsl(var(--rex-signal,140_90%_50%))]">
                          +{c.xp_reward} XP
                        </div>
                ...
                <div key={r.id ?? `lb-${i}`} className="flex items-center gap-3 p-3">
                  <div
                    className={`w-6 text-center text-sm font-bold ${
                      i === 0
                        ? "text-[hsl(var(--rex-signal,140_90%_50%))]"
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-[11px] text-muted-foreground">
                      @{r.handle} · L{r.level ?? 1}
                    </div>
                  </div>
                  <div className="text-right">
                    <div className="text-sm font-semibold">
                      {(r.xp_total ?? 0).toLocaleString()}
                    </div>
