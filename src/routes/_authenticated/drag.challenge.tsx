...
export const Route = createFileRoute("/_authenticated/drag/challenge")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Send a real head-to-head drag race challenge and race another rider live on ZOMBIEREX." },
    ],
  }),
...
function DragChallengePage() {
  ...
  const send = useMutation({
    mutationFn: () => createFn({ data: {
      opponent_handle: handle, strip_mode: strip, tree_mode: tree, stake_xp: stake,
      message: message || null,
    }}),
    onSuccess: () => { toast.success("Challenge sent"); setHandle(""); setMessage(""); qc.invalidateQueries({ queryKey: ["drag", "challenges"] }); },
    onError: (e: Error) => toast.error(e.message),
  });

  const respond = useMutation({
    mutationFn: (v: { id: string; action: "accept" | "decline" | "cancel" }) => respondFn({ data: v }),
    onSuccess: (r: any) => {
      if (r?.status === "accepted" && r.match_id) nav({ to: "/drag/match/$id", params: { id: r.match_id } });
    ...
    <div className="min-h-svh pb-24">
      <div className="px-4 pt-4">
        <div className="flex items-center gap-2">
          <Link to="/drag" className="tap mono-caps text-[10px]" style={{ color: "var(--color-silver)" }}>← DRAG</Link>
        </div>
        <h1 className="serif mt-2 text-3xl" style={{ color: "var(--color-ink)" }}>Challenge a Rider</h1>
        ...
        <section className="mt-5 rounded-2xl border p-4"
          style={{ borderColor: "rgba(0,200,83,0.35)", background: "linear-gradient(120deg,#050505,#0f2015)" }}>
          <p className="mono-caps text-[10px] font-black" style={{ color: "#00c853", letterSpacing: "0.24em" }}>◆ NEW CHALLENGE</p>
          <label className="mt-3 block">
            <span className="mono-tag" style={{ color: "var(--color-silver)", fontSize: 9 }}>OPPONENT HANDLE</span>
            <input value={handle} onChange={(e) => setHandle(e.target.value)} placeholder="@rider"
              className="mt-1 w-full rounded-md border px-3 py-2 text-sm outline-none"
              style={{ borderColor: "var(--color-hair)", background: "#0a0a0a", color: "#f5f5f5" }} />
          </label>
          <div className="mt-3 grid grid-cols-2 gap-3">
            <label>
              <span className="mono-tag" style={{ color: "var(--color-silver)", fontSize: 9 }}>STRIP</span>
              <select value={strip} onChange={(e) => setStrip(e.target.value as any)}
            ...
            <label>
              <span className="mono-tag" style={{ color: "var(--color-silver)", fontSize: 9 }}>TREE</span>
              <select value={tree} onChange={(e) => setTree(e.target.value as any)}
                className="mt-1 w-full rounded-md border px-3 py-2 text-sm"
                style={{ borderColor: "var(--color-hair)", background: "#0a0a0a", color: "#f5f5f5" }}>
          ...
          <label className="mt-3 block">
            <span className="mono-tag" style={{ color: "var(--color-silver)", fontSize: 9 }}>STAKE (XP)</span>
            <input type="number" min={0} max={5000} value={stake}
              onChange={(e) => setStake(Math.max(0, Math.min(5000, Number(e.target.value) || 0)))}
              className="mt-1 w-full rounded-md border px-3 py-2 text-sm"
              style={{ borderColor: "var(--color-hair)", background: "#0a0a0a", color: "#f5f5f5" }} />
          </label>
          <label className="mt-3 block">
            <span className="mono-tag" style={{ color: "var(--color-silver)", fontSize: 9 }}>MESSAGE (OPTIONAL)</span>
          ...
          <button
            ...
            style={{ padding: "12px", background: "var(--color-neon)", color: "var(--color-obsidian)", opacity: (!handle.trim() || send.isPending) ? 0.5 : 1 }}>
        ...
        <Section title="RECEIVED" empty="No challenges waiting for you.">
          {received.map((c: any) => (
            <ChallengeCard key={c.id} c={c}
              actions={
                <>
                  <button onClick={() => respond.mutate({ id: c.id, action: "accept" })}
                    className="tap mono-caps text-[10px] font-black"
                    style={{ padding: "8px 12px", background: "var(--color-neon)", color: "var(--color-obsidian)" }}>ACCEPT ▶</button>
                  <button onClick={() => respond.mutate({ id: c.id, action: "decline" })}
                    className="tap mono-caps text-[10px] font-black"
                    style={{ padding: "8px 12px", background: "#111", color: "#f5f5f5", border: "1px solid #333" }}>DECLINE</button>
                </>
              } />
          ))}
        </Section>
        ...
        <Section title="SENT" empty="Nothing pending on your side.">
          {sent.map((c: any) => (
            <ChallengeCard key={c.id} c={c}
              actions={
                <button onClick={() => respond.mutate({ id: c.id, action: "cancel" })}
                  className="tap mono-caps text-[10px] font-black"
                  style={{ padding: "8px 12px", background: "#111", color: "#f5f5f5", border: "1px solid #333" }}>CANCEL</button>
              } />
          ))}
        </Section>
        ...
        <Section title="HISTORY" empty="No past challenges yet.">
          {history.slice(0, 20).map((c: any) => (
            <ChallengeCard key={c.id} c={c}
              actions={c.match_id ? (
                <Link to="/drag/match/$id" params={{ id: c.match_id }}
                  className="tap mono-caps text-[10px] font-black"
                  style={{ padding: "8px 12px", background: "#111", color: "var(--color-neon)", border: "1px solid rgba(0,200,83,0.4)" }}>OPEN</Link>
              ) : null} />
          ))}
        </Section>
      </div>
...
function Section({ title, empty, children }: { title: string; empty: string; children: React.ReactNode }) {
  ...
  return (
    <section className="mt-6">
      <h2 className="mono-caps text-[10px] font-black" style={{ color: "var(--color-silver)" }}>{title}</h2>
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="mono-tag" style={{ color: "var(--color-silver)", fontSize: 9 }}>
            {c.strip_mode === "quarter" ? "1/4 MI" : "1/8 MI"} · {c.tree_mode.toUpperCase()} · STAKE {c.stake_xp} XP · {c.status.toUpperCase()}
          </p>
          {c.message && <p className="mt-1 text-xs" style={{ color: "var(--color-ink-3)" }}>&ldquo;{c.message}&rdquo;</p>}
        </div>
