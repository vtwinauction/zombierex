    ...
    <PullToRefresh onRefresh={() => refetch()}>
      <div className="pb-24">
        ...
        {isLoading && (
          <div className="px-4 pt-6 space-y-3">
            {[0, 1, 2].map((i) => (
              <div
                key={i}
                className="h-24 w-full animate-pulse hairline"
        ...
        {!isLoading && events.length === 0 && (
          <div
            className="mx-4 mt-6 border border-dashed p-8 text-center"
            style={{ borderColor: "var(--color-hair-strong)" }}
          >
            <Calendar size={32} className="mx-auto" style={{ color: "var(--color-neon)" }} />
            <p className="mono-tag mt-3" style={{ color: "var(--color-titanium)" }}>
              NO EVENTS HERE
            </p>
            <p className="mt-2 text-sm" style={{ color: "var(--color-ash)" }}>
              Browse events or host your own.
            </p>
            <Link
              to="/events"
              ...
              <div className="min-w-0 flex-1">
                ...
                >
                  <span className="flex items-center gap-1">
                    <Users size={12} /> {e.rsvp_count ?? 0}
                  </span>
                  {e.my_rsvp && (
                    <span
                      className="mono-tag rounded px-1.5 py-0.5"
                      style={{ background: "rgba(0,200,83,0.12)", color: "var(--color-neon)" }}
                    >
