...
function PlanPage() {
  ...
  function exportDraftGpx() {
    if (path.length < 2) {
      setErr("Add at least 2 waypoints");
      return;
    }
  ...
  async function findPlaces() {
    if (path.length === 0) {
      setErr("Add at least one waypoint first");
      return;
    }
    setErr(null);
    setSearching(true);
    try {
      ...
      const res = await searchPois({
        data: { lat: mid.lat, lng: mid.lng, kind: searchKind as any },
      });
      ...
      <div className="px-4 pt-3 space-y-3">
        ...
        <div className="flex flex-wrap gap-2">
          <button
            ...
            className="tap mono-caps text-[10px] font-bold px-3 py-2 text-white/80 disabled:opacity-30"
          ...
          <button
            ...
            className="tap mono-caps text-[10px] font-bold px-3 py-2 text-white/80"
            style={{ border: "1px solid var(--color-hair-strong)" }}
          >
            IMPORT GPX
          </button>
          <button
            onClick={exportDraftGpx}
        ...
        <input
          ...
          className="w-full bg-graphite p-3 text-white border border-white/10"
        />
        <textarea
          value={description}
        ...
        <input
          ...
          placeholder="Region (e.g. Hajar Mountains)"
          className="w-full bg-graphite p-3 text-white border border-white/10"
        />
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <Select
            label="SURFACE"
            value={surface}
            onChange={(v) => setSurface(v as any)}
            options={SURFACES as unknown as string[]}
          />
        </div>
        <Select
          label="VISIBILITY"
        ...
        <div className="mt-2 border-t border-white/10 pt-4">
          <p className="mono-caps text-[10px] font-bold" style={{ color: "var(--color-neon)" }}>
          ...
          <button
            onClick={findPlaces}
            disabled={searching || path.length === 0}
            className="mt-2 tap mono-caps text-[10px] font-bold px-3 py-2 text-white disabled:opacity-40"
            style={{
            ...
            <div className="mt-2 max-h-60 overflow-auto border border-white/10">
              {searchResults.map((r, i) => (
                <button
                  key={i}
                  onClick={() => addPoi(r)}
                  className="tap block w-full text-left p-2 border-b border-white/5 hover:bg-white/5"
                >
          ...
          {pois.length > 0 && (
            <div className="mt-3 space-y-1">
              ...
              {pois.map((p, i) => (
                <div
                  key={i}
                  className="flex items-center gap-2 border border-white/10 bg-graphite px-2 py-1"
                >
        ...
        <button
          onClick={save}
          disabled={saving}
          className="tap w-full py-3 mono-caps text-sm font-bold disabled:opacity-50"
  ...
  return (
    <label className="block">
      ...
      <select
        ...
        className="mt-1 w-full bg-graphite p-3 text-white border border-white/10"
      >
        {options.map((o) => (
