...
export const Route = createFileRoute("/atlas")({
  head: () => ({
    meta: [
      { title: "Route Atlas — Zombierex" },
      {
        name: "description",
        content:
          "Discover, save, and share motorcycle routes with hotels, restaurants, and scenic stops.",
      },
      { property: "og:title", content: "Route Atlas — Zombierex" },
      {
        property: "og:description",
        content: "Rider-planned routes with POIs. Ride them, save them, share them.",
      },
    ],
  }),
  ...
  errorComponent: ({ error }) => (
    <div className="min-h-svh grid place-items-center p-6 text-sm">
...
function AtlasPageInner() {
  ...
  const [geoStatus, setGeoStatus] = useState<"idle" | "locating" | "ok" | "denied" | "unsupported">(
    "idle",
  );
  ...
  const filtered = useMemo(() => {
    ...
    return routes.filter(
      (r: any) => r.title?.toLowerCase().includes(q) || r.region?.toLowerCase().includes(q),
    );
  }, [routes, query]);
  ...
  return (
    <div className="fixed inset-0 overflow-hidden bg-background">
          ...
          <RouteMap
            ...
            onCommunityPoiClick={(p) => {
              ...
              if (found)
                toast(found.name, {
                  description:
                    found.note ?? found.address ?? `${found.kind.toUpperCase()} · added by rider`,
                });
            }}
            onMapClick={
              dropMode
      ...
      >
        <Link
          to="/atlas/ride"
          aria-label="Ride Mode — turn-by-turn navigation"
        ...
        <Link
          to="/atlas/cockpit"
          aria-label="Cockpit — full-screen HUD"
        ...
        <Link
          to="/atlas/voice"
          aria-label="Hey Rex voice commands"
          className="tap grid h-12 w-12 place-items-center rounded-full bg-card border border-border shadow-lg"
          style={{ color: "var(--color-ink-0)" }}
        >
        ...
        <button
          ...
          className="tap grid h-12 w-12 place-items-center rounded-full bg-card border border-border shadow-lg"
          style={{
            color: geoStatus === "ok" ? "var(--color-neon-deep, #4b8f00)" : "var(--color-ink-0)",
          }}
        >
          <Locate
            size={18}
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 pb-2">
          <div className="min-w-0">
            <p className="serif text-xl italic text-foreground leading-tight truncate">
              Route Atlas
            </p>
        ...
        {sheet === "peek" ? (
          ...
          <div
            ...
            style={{ height: `calc(${sheetHeight[sheet]} - 88px)` }}
          >
            {filtered.length === 0 ? (
              <EmptyState />
            ) : (
    ...
    >
      <div className="relative h-20 w-24 shrink-0 overflow-hidden rounded-xl bg-muted">
        ...
        <div className="absolute inset-x-1 bottom-1 text-center">
          <span
            className="mono-num text-[10px] font-bold text-white"
      ...
      <div className="min-w-0 flex-1">
        <p
          className="mono-caps text-[9px] font-bold"
          style={{
        ...
        <p className="truncate text-sm font-semibold text-foreground leading-tight mt-1">
          {route.title}
        </p>
        <p className="mono-tag mt-1 truncate text-muted-foreground" style={{ fontSize: 9 }}>
          {route.region ? `${route.region} · ` : ""}
          {hrs > 0 ? `${hrs}h ` : ""}
          {mins}m · ★ {route.saves_count} · ▶ {route.rides_count}
        </p>
        <button
          onClick={(e) => {
            e.stopPropagation();
...
function AtlasStatsStrip({
  ...
  return (
    <div
      className="mx-4 mb-3 overflow-hidden rounded-2xl border border-border"
      style={{
        background:
          "linear-gradient(180deg, hsl(var(--card)), color-mix(in oklab, var(--color-neon) 6%, hsl(var(--muted))))",
      }}
    >
      <div className="grid grid-cols-2 divide-x divide-border">
        <div className="px-4 py-3">
          <p className="mono-caps text-[8px] tracking-[0.2em] text-muted-foreground">
            {route ? "DISTANCE" : "ROUTES"}
          </p>
          <p
            className="mono-num text-3xl font-black leading-none tabular-nums"
            style={{ color: "var(--color-neon-deep)" }}
          >
            {km}
            <span className="mono-caps ml-1 text-[10px] font-bold text-muted-foreground">
              {route ? "KM" : "LIVE"}
            </span>
          </p>
        </div>
        <div className="px-4 py-3">
          <p className="mono-caps text-[8px] tracking-[0.2em] text-muted-foreground">
            {route ? "RIDE TIME" : "COMMUNITY STOPS"}
          </p>
          <p
            className="mono-num text-3xl font-black leading-none tabular-nums"
            style={{ color: "var(--color-neon-deep)" }}
...
function Legend({ Icon, label, color }: { Icon: typeof Hotel; label: string; color: string }) {
  return (
    <span className="inline-flex items-center gap-1.5">
      ...
      <span className="mono-caps text-[8px] font-bold tracking-[0.16em] text-muted-foreground">
        {label}
      </span>
    </span>
  );
...
function EmptyState() {
  return (
    <div className="rounded-2xl border border-dashed border-border p-8 text-center bg-muted/30">
      ...
      <p className="mono-tag mt-1 text-muted-foreground" style={{ fontSize: 10 }}>
        Try clearing filters or plan the first one.
      </p>
      <Link
        to="/atlas/new"
        className="tap mt-3 inline-flex items-center gap-1.5 mono-caps text-[10px] font-bold"
        style={{
          padding: "8px 14px",
          background: "var(--color-neon)",
          color: "var(--color-obsidian)",
...
function BluetoothChip() {
  ...
  async function onPair() {
    ...
    try {
      ...
      try {
        localStorage.setItem("zrex:bt", JSON.stringify({ name, at: Date.now() }));
      } catch {
        /* noop */
      }
      setState("linked");
    } catch {
      setState((p) => (p === "linked" ? "linked" : "idle"));
    }
  }
  ...
  return (
    <button
      ...
      style={{
        background: linked
          ? "color-mix(in oklab, var(--color-neon) 20%, hsl(var(--card)))"
          : "hsl(var(--card) / 0.9)",
...
function DropPointCard({
  ...
  return (
    <div
      className="absolute inset-x-3 z-40 rounded-2xl border border-border bg-card p-4 shadow-2xl"
      style={{ bottom: "calc(18svh + 16px)" }}
    >
      <div className="flex items-center justify-between mb-2">
        <p className="text-sm font-semibold text-foreground">Drop a community point</p>
        <button onClick={onCancel} className="tap text-muted-foreground" aria-label="Cancel">
          <X size={16} />
        </button>
      </div>
      ...
      <div className="mt-3 flex items-center justify-end gap-2">
        <button
          onClick={onCancel}
          className="tap rounded-full border border-border px-3 py-1.5 text-xs font-semibold text-foreground"
        >
          Cancel
        </button>
        <button
