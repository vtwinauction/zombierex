...
function SellerPage() {
  ...
  const { data: reviews } = useQuery({
    ...
    queryFn: () => revFn({ data: { seller_id: id, limit: 20 } }),
  });
  ...
  return (
    ...
    >
      <div className="pb-24">
        <div className="px-4 pt-6">
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1 min-w-0">
              <h1 className="serif text-2xl italic truncate" style={{ color: "var(--color-ink)" }}>
                {p?.display_name ?? p?.handle ?? "Seller"}
              </h1>
              ...
              {p?.tier && (
                <span
                  className="mono-tag mt-1 inline-block px-2 py-0.5"
        ...
        <div
          className="mx-4 mt-6 grid grid-cols-4 border"
              ...
              >
                <div className="flex items-center justify-between">
                  <span className="text-sm font-bold" style={{ color: "var(--color-ink)" }}>
                    {r.reviewer?.display_name ?? r.reviewer?.handle ?? "Rider"}
                  </span>
                  <span className="mono-num" style={{ color: "var(--color-neon)" }}>
                    ★ {r.rating}
                  </span>
                </div>
                {r.body && (
