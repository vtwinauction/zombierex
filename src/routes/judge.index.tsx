...
const eventsQ = queryOptions({
  queryKey: ["judge-events", "all"],
  queryFn: () => judgeListEvents({ data: { status: "all" } }),
});
  ...
  head: () => ({
    meta: [
      { title: "AI Show Judge · ZOMBIEREX" },
      {
        name: "description",
        content:
          "AI-powered concours judging for motorcycles and cars — enter shows, get scored, win awards.",
      },
      { property: "og:title", content: "AI Show Judge · ZOMBIEREX" },
      {
        property: "og:description",
        content: "AI-powered concours judging for motorcycles and cars.",
      },
      ...
      {!flag.enabled && (
        <div className="mx-5 mt-6 p-4 surface-1 lift-1" style={{ borderRadius: 10 }}>
          ...
          <p className="mt-1 text-[13px]" style={{ color: "var(--color-ink)" }}>
            AI Show Judge is currently disabled by administrators. New entries are paused; existing
            published events remain viewable.
          </p>
        </div>
      )}
            ...
            {events.map((e) => (
              ...
              >
                <div className="flex items-center justify-between">
                  <p className="serif text-lg" style={{ color: "var(--color-ink)" }}>
                    {e.title}
                  </p>
                  <span
                    className="mono-tag"
                    style={{
                      color: e.status === "published" ? "var(--color-neon)" : "var(--color-silver)",
                    }}
                  >
                ...
                <div
                  className="mt-2 flex gap-3 mono-tag"
                  style={{ color: "var(--color-silver)", fontSize: 10 }}
                >
                  <span>{(e.vehicle_types ?? []).join(" · ").toUpperCase()}</span>
                  {e.registration_closes_at && (
                    <span>REG · {new Date(e.registration_closes_at).toLocaleDateString()}</span>
                  )}
                </div>
              </Link>
            ))}
