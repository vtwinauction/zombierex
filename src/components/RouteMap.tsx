...
function loadGoogleMaps(): Promise<any> {
  ...
  const key =
    ...
    GOOGLE_MAPS_BROWSER_KEY_FALLBACK;
  ...
  function drawPois(g: any) {
    ...
    pois.forEach((p) => {
      const m = new g.maps.Marker({
        ...
        title: p.name ?? "",
        label: p.kind
          ? { text: iconForKind(p.kind), color: "#0b0d10", fontSize: "12px", fontWeight: "700" }
          : undefined,
        icon: {
  ...
  function drawCommunity(g: any) {
    ...
    communityPois.forEach((p) => {
      const m = new g.maps.Marker({
        ...
        title: p.name ?? "",
        label: p.kind
          ? { text: iconForKind(p.kind), color: "#ffffff", fontSize: "11px", fontWeight: "700" }
          : undefined,
        icon: {
          path: g.maps.SymbolPath.CIRCLE,
          scale: 11,
...
function FallbackRouteMap({
  ...
  return (
    ...
    >
      ...
      >
        <path
          d="M0 28 C20 22 32 34 50 28 S78 16 100 24"
          fill="none"
          stroke="rgba(8,13,10,0.09)"
          strokeWidth="5"
        />
        <path
          d="M0 66 C18 58 34 72 52 62 S80 50 100 58"
          fill="none"
          stroke="rgba(8,13,10,0.08)"
          strokeWidth="4"
        />
        <path
          d="M18 0 C24 26 17 45 29 100"
          fill="none"
          stroke="rgba(8,13,10,0.06)"
          strokeWidth="3"
        />
        <path
          d={routeD}
          fill="none"
          stroke="rgba(0,200,83,0.98)"
          strokeWidth="1.4"
          strokeLinecap="round"
        ...
        {userLocation &&
          (() => {
            ...
            return (
              <path
                d={`M${pt.x} ${pt.y - 2.4} L${pt.x + 1.9} ${pt.y + 2.1} L${pt.x} ${pt.y + 0.9} L${pt.x - 1.9} ${pt.y + 2.1} Z`}
                fill="#00c853"
                stroke="#0b0d10"
                strokeWidth="0.5"
                vectorEffect="non-scaling-stroke"
